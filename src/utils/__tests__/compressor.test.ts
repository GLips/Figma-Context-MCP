...
import { shortenGlobalVarIds } from '../compressor.js';
  ...
  it('should shorten globalVar IDs correctly', () => {
    ...
    const globalVars: GlobalVars = {
      styles: {
        ...
        "fill_PNVRKQ": [{ type: "IMAGE", imageRef: "71327eae4d7fe3edf4f767464be4b5e7a87aff6c", scaleMode: "FILL" }],
        "style_WMXDFX": {
          fontFamily: "Urbanist",
          fontWeight: 400,
          fontSize: 24,
  ...
  it('should handle real Figma data sample', () => {
    ...
    styleKeys.forEach(originalId => {
      ...
      if (match && match[1]) {
        ...
        expect(originalPrefix.startsWith(shortPrefix) || shortPrefix + '_fallback' === originalPrefix).toBeTruthy();
      }
    });
  });
});
