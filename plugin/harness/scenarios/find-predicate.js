...
const whites = await flcm.find(
  { type: "RECTANGLE" },
  (n) => Array.isArray(n.fills) && n.fills[0] === "#FFFFFF",
);
...
const faded = await flcm.find(
  { type: "RECTANGLE" },
  (n) => n.opacity !== undefined && n.opacity < 1,
);
...
// not the rects.
const whiteFrames = await flcm.find(
  { type: "FRAME" },
  (n) => Array.isArray(n.fills) && n.fills[0] === "#FFFFFF",
);
