...
const card = ({ type: "FRAME", ...({
    key: "card",
    width: 320,
    fill: "linear-gradient(180deg, #6366f1, #a855f7)",
    layout: { mode: "column", gap: 12, padding: 24 },
  }), children: [
    ({ type: "TEXT", text: "Hello **world**", ...({ key: "title", textStyle: { fontSize: 18 } }) }),
    ({ type: "RECTANGLE", ...({ key: "chip", width: 48, height: 48, fill: "#22c55e", borderRadius: 8 }) }),
  ] });
...
return { type: node.type, mode: node.layout && node.layout.mode, children: (node.children || []).length };
