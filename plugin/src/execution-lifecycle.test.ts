...
const code = buildSync({ entryPoints: [new URL("./code.ts", import.meta.url).pathname], bundle: true, write: false, format: "iife" }).outputFiles[0].text;
const flush = async () => { for (let n = 0; n < 30; n++) await Promise.resolve(); };
function host() {
  const frames: Record<string, unknown>[] = [];
  const figma = {
    showUI() {}, notify() {}, currentPage: { name: "test" },
    clientStorage: { getAsync: async () => [], setAsync: async () => {} },
    ui: { postMessage: (msg: Record<string, unknown>) => frames.push(msg), onmessage: (_msg: Record<string, unknown>) => {}, resize() {} },
  };
...
test("queued cancellation never executes, old settlement cannot cancel or erase replacement run", async () => {
  const h = host(); await h.connect(1);
  let release!: () => void;
  h.context.hold = new Promise<void>((r) => { release = r; });
  h.context.writes = [];
  const run = (key: number, id: string, body: string) => h.send({ type: "EXECUTE_CODE", id, __connKey: key, preamble: "(host)=>({})", code: body });
  ...
  assert(h.frames.some((f) => f.type === "RUN_STATE" && f.runId === "same-id" && f.__connKey === 2 && f.phase === "running"));
  assert(h.frames.some((f) => f.id === "queued" && String(f.errors).includes("no code was executed")));
});
...
test("Reject is explicit and unknown tokens cannot bypass approval status or execution", async () => {
  ...
  h.context.writes = [];
  h.send({ type: "EXECUTE_CODE", id: "run", __connKey: 1, preamble: "(host)=>({})", code: "writes.push('bad');" });
  await flush(); assert.equal(h.context.writes.length, 0);
});
...
test("actual UI routes old results and image requests only to their original connection", () => {
  ...
  const script = readFileSync(new URL("../ui.html", import.meta.url), "utf8").split("<script>")[1].split("</script>")[0];
  const context = vm.createContext({ WebSocket: Socket, parent: { postMessage: (msg: { pluginMessage: Record<string, unknown> }) => forwarded.push(msg) }, window: {}, document: { getElementById: () => ({}) }, setTimeout() {}, requestAnimationFrame() {}, console });
  vm.runInContext(script, context);
  const old: Socket = context.sockets[9876]; old.onopen();
  const oldKey = forwarded.at(-1)!.pluginMessage.__connKey;
  context.connect(9876);
  const next: Socket = context.sockets[9876]; next.onopen();
  ...
  context.window.onmessage({ data: { pluginMessage: { type: "EXECUTE_CODE_RESULT", __connKey: newKey, result: "new" } } });
