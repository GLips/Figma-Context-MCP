...
test("four bounds survive creation, canonical/plugin reads, edits, and explicit clearing", async () => {
  ...
  await edit(id(node.id), {
    minWidth: "none",
    maxWidth: "none",
    minHeight: "none",
    maxHeight: "none",
  });
  assert.deepEqual(boundsOf(node), {
    minWidth: null,
    maxWidth: null,
    minHeight: null,
    maxHeight: null,
  ...
  assert.deepEqual(
    boundsOf((await simplify([await sceneNodeToSnapshot(node, async () => null)])).nodes[0]),
    Object.fromEntries(boundKeys.map((key) => [key, undefined])),
  );
});
...
test("same-edit bounds clamp successfully and warnings report requested, bound and actual values", async () => {
  ...
  for (const [key, axis, bound, requested] of [
    ["minWidth", "width", 160, 145],
    ["maxWidth", "width", 240, 260],
...
test("contradictory and malformed bounds reject before batch writes, valid interval moves succeed", async () => {
  ...
  for (const value of [0, -1, NaN, Infinity, "20px"])
    await assert.rejects(edit(built, { maxHeight: value } as any), /finite positive/);
  await assert.rejects(
    render({ type: "FRAME", minHeight: 80, maxHeight: 40 }),
    /minHeight 80 exceeds maxHeight 40/,
  );
    ...
    children: [
      {
        ...
        height: 100,
        clip: true,
        children: [{ type: "RECTANGLE", width: 150, height: 20 }],
      },
      {
        type: "FRAME",
        key: "outer",
        ...
        children: [
          {
            ...
            width: 80,
            height: 40,
            children: [{ type: "RECTANGLE", width: 100, height: 20 }],
          },
        ],
      },
      {
    ...
    children: [
      {
        ...
        width: "fill",
        height: 80,
        children: [{ type: "RECTANGLE", width: 180, height: 20 }],
      },
    ],
  });
  warnings = [];
