...
import { frame, rect, instance, id } from "./flcm.js";
...
test("four bounds survive creation, canonical/plugin reads, edits, and explicit clearing", async () => {
  ...
  const built = await render(frame({ width: 200, height: 60, layout: { mode: "row" }, ...bounds }));
  const node = await figma.getNodeByIdAsync(built.node.id);
  assert.deepEqual(boundsOf(built.node), bounds);
...
test("same-edit bounds clamp successfully and warnings report requested, bound and actual values", async () => {
  const built = await render(frame({ width: 200, height: 60, layout: { mode: "row" } }));
  for (const [key, axis, bound, requested] of [
    ...
    warnings = [];
    const handle = await edit(built.node, { [key]: bound, [axis]: requested, opacity: 0.5 });
...
test("instance-only bounds persist through resize and clear without changing the component", async () => {
  ...
  const built = await render(instance(component.id));
  const node = await figma.getNodeByIdAsync(built.node.id);
  await edit(built.node, { minWidth: 160 });
  warnings = [];
  const resized = await edit(built.node, { width: 145 });
  ...
  await edit(built.node, { minWidth: "none", width: 145 });
...
test("contradictory and malformed bounds reject before batch writes, valid interval moves succeed", async () => {
  const built = await render(frame({ width: 80, height: 80, layout: { mode: "row" }, minWidth: 20, maxWidth: 100 }));
  const node = await figma.getNodeByIdAsync(built.node.id);
  const before = [...figma.undoLog];
  await assert.rejects(editMany([{ target: built.node, changes: { minWidth: 120, opacity: 0.5 } }]), /minWidth 120 exceeds maxWidth 100/);
  assert.equal(node.opacity, 1);
  assert.deepEqual(figma.undoLog, before);
  for (const value of [0, -1, NaN, Infinity, "20px"]) await assert.rejects(edit(built.node, { maxHeight: value } as any), /finite positive/);
  assert.throws(() => frame({ minHeight: 80, maxHeight: 40 }), /minHeight 80 exceeds maxHeight 40/);
  await edit(built.node, { minWidth: 120, maxWidth: 200, width: 150 });
...
test("overflow warning deduplicates clipped/unclipped containers, includes preexisting nested overflow, and excludes unaffected siblings", async () => {
  const built = await render(frame({ width: 500, height: 300 }, [
    frame({ key: "clipped", width: 100, height: 100, clip: true }, [rect({ width: 150, height: 20 })]),
    frame({ key: "outer", width: 200, height: 100, left: 200 }, [frame({ key: "nested", width: 80, height: 40 }, [rect({ width: 100, height: 20 })])]),
    frame({ key: "unaffected", width: 100, height: 100, top: 150 }, [rect({ width: 150, height: 20 })]),
  ]));
  warnings = [];
  const handles = await editMany([
    { target: built.keyed.clipped, changes: { width: 90 } },
    { target: built.keyed.outer, changes: { width: 190 } },
    { target: built.keyed.nested, changes: { height: 35 } },
  ]);
  ...
  assert.ok(overflow[0].includes(built.keyed.clipped.id));
  assert.ok(overflow[0].includes(built.keyed.nested.id));
  assert.ok(!overflow[0].includes(built.keyed.unaffected.id));
  assert.equal(overflow[0].split(built.keyed.nested.id).length, 2);
  warnings = [];
  await edit(built.keyed.clipped, { opacity: 0.5 });
  assert.equal(warnings.length, 0);
});

test("overflow is measured after all batch sizes settle", async () => {
  const built = await render(frame({ width: 200, height: 100 }, [rect({ key: "child", width: 180, height: 40 })]));
  warnings = [];
  await editMany([{ target: built.node, changes: { width: 100 } }, { target: built.keyed.child, changes: { width: 80 } }]);
...
test("a resized parent's nested fill layout is inspected after reflow", async () => {
  const built = await render(frame({ width: 200, height: 100, layout: { mode: "row" } }, [
    frame({ key: "nested", width: "fill", height: 80 }, [rect({ width: 180, height: 20 })]),
  ]));
  warnings = [];
  await edit(built.node, { width: 100 });
  const overflow = warnings.filter(message => message.includes(": overflow in"));
  assert.equal(overflow.length, 1);
  assert.ok(overflow[0].includes(built.keyed.nested.id));
});
...
test("overflow uses container-local geometry when the container is rotated", async () => {
  const built = await render(frame({ width: 100, height: 100 }, [rect({ key: "child", width: 20, height: 20 })]));
  const root = await figma.getNodeByIdAsync(built.node.id);
  const child = await figma.getNodeByIdAsync(built.keyed.child.id);
