...
test("aliases normalize compilers and edits without changing canonical output", async () => {
  ...
  const built = await render(({ type: "FRAME", clipsContent: true, clip: true, children: [({ type: "TEXT", text: "Copy", fontSize: 24, textStyle: { fontSize: 24 } })] }));
  ...
  assert.throws(() => compileTree(({ type: "FRAME", clip: false, clipsContent: true }), "spec"), /conflicting clipsContent and clip/);
  assert.throws(() => compileTree(({ type: "TEXT", text: "x", fontSize: 12, textStyle: { fontSize: 13 } }), "spec"), /conflicting fontSize and textStyle.fontSize/);
...
test("regex searches preserve flags and caller lastIndex, strings remain literal", async () => {
  createFigmaMock();
  await render(({ type: "FRAME", children: [({ type: "TEXT", text: "a", name: "Copy" }), ({ type: "TEXT", text: "b", name: "copy" }), ({ type: "RECTANGLE", name: "/Copy/" })] }));
...
test("insertion inherits selected component layout; non-auto controls reject before writes", async () => {
  const figma = createFigmaMock();
  const comp = await component(({ type: "FRAME", layout: { mode: "row" }, children: [({ type: "TEXT", text: "label" })] }));
  const parent = await render(({ type: "FRAME" }));
  for (const insert of [append, prepend]) {
    const result = await insert(parent.id, ({ type: "INSTANCE", componentId: comp.id, layout: { gap: 9, padding: 4 } }));
  ...
  await assert.rejects(append(parent.id, ({ type: "INSTANCE", componentId: plain.id, layout: { gap: 9 } })), /need an auto-layout/);
  assert.equal(node.children.length, count);
});

test("baseline authors and edits rows and rejects columns before writes", async () => {
  const figma = createFigmaMock();
  const result = await render(({ type: "FRAME", layout: { mode: "row", alignItems: "baseline" }, children: [({ type: "TEXT", text: "small", fontSize: 12 }), ({ type: "TEXT", text: "large", fontSize: 24 }), ({ type: "RECTANGLE", width: 10, height: 10 })] }));
  const node = await figma.getNodeByIdAsync(result.id);
  ...
  assert.throws(() => compileTree(({ type: "FRAME", layout: { mode: "column", alignItems: "baseline" } }), "spec"), /baseline.*requires/);
});
...
test("selected variants and slot content inherit resolved auto layout", async () => {
  ...
  const plain = await component(({ type: "FRAME" }));
  const set = await variants([{ component: row.id, variant: { Mode: "Row" } }, { component: plain.id, variant: { Mode: "Plain" } }], { name: "Modes" });
  const changing = await render(({ type: "INSTANCE", componentId: set.id, componentProperties: { Mode: "Plain" } }));
  await edit(changing.id, { componentProperties: { Mode: "Row" }, layout: { alignItems: "baseline" } });
  assert.equal((await figma.getNodeByIdAsync(changing.id)).counterAxisAlignItems, "BASELINE");
  await assert.rejects(edit(changing.id, { componentProperties: { Mode: "Plain" }, layout: { alignItems: "baseline" } }), /baseline.*requires/);
  const holder = await component(({ type: "FRAME", children: [({ type: "FRAME", key: "slot", componentPropertyReferences: { slot: "Body" } })] }), { propertyDefinitions: { Body: { type: "slot" } } });
  const content = () => ({ type: "INSTANCE", componentId: set.id, componentProperties: { Mode: "Row" }, layout: { gap: 8, alignItems: "baseline" } });
  const out = await render(({ type: "INSTANCE", componentId: holder.id, overrides: { [specNode(holder, "slot").id]: { children: [content()] } } }));
  ...
  await assert.rejects(append(live.children[0].id, ({ type: "INSTANCE", componentId: set.id, componentProperties: { Mode: "Plain" }, layout: { gap: 8 } })), /need an auto-layout/);
...
test("shipped factory supports RegExp input and promotion aliases across separate evaluations", async () => {
  ...
  install();
  await runInContext('(async () => { var built = await flcm.render({type:"FRAME", name:"Copy"}); globalThis.oldId = built.id; globalThis.newId = (await flcm.component(oldId)).id; })()', context);
  install();
