...
test("textTransform and fontVariant are two CSS words for ONE Figma slot", async () => {
  ...
  await assert.rejects(
    render({
      type: "TEXT",
      text: "x",
      textStyle: { textTransform: "uppercase", fontVariant: "small-caps" },
    }),
    /textTransform and fontVariant are two CSS words for Figma's ONE textCase slot/,
  );
  const smallCaps = await render({
    type: "TEXT",
    text: "x",
    textStyle: { fontVariant: "all-small-caps" },
  });
...
test("a run delta carries casing and paragraph metrics over its own slice", async () => {
  const out = await render({
    type: "TEXT",
    text: ["plain ", ["SHOUT", { textTransform: "uppercase", paragraphSpacing: 20 }]],
  });
  const node = await figma.getNodeByIdAsync(out.id);
...
test("a read textStyle renders derived font identity and refuses unwritable fields by name", async () => {
  ...
  await assert.rejects(
    render({ type: "TEXT", text: "x", textStyle: { fontVariantname: "Bold" } as never }),
    /unknown prop "fontVariantname"/,
  );
});
...
test("a hyperlink takes the author string or the read form; a NODE link has no authored form", async () => {
  const out = await render({
    type: "TEXT",
    text: "docs",
    textStyle: { hyperlink: { type: "URL", url: "https://x.co" } },
  });
  const node = await figma.getNodeByIdAsync(out.id);
  ...
  await assert.rejects(
    render({
      type: "TEXT",
      text: "x",
      textStyle: { hyperlink: { type: "NODE", nodeID: "1:2" } } as never,
    }),
