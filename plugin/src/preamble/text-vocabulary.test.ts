...
// back into an edit or rebuilt through the compilers. Two concerns pinned here — that each word actually LANDS
...
test("the base text words land on the node — casing, vertical align, paragraph metrics, whole-node link", async () => {
  const out = await render(
    ({ type: "TEXT", text: "terms and conditions", textStyle: {
...
test("textTransform and fontVariant are two CSS words for ONE Figma slot", async () => {
  ...
  assert.throws(
    () => compileTree(({ type: "TEXT", text: "x", textStyle: { textTransform: "uppercase", fontVariant: "small-caps" } }), "spec"),
    /textTransform and fontVariant are two CSS words for Figma's ONE textCase slot/,
  );
  const smallCaps = await render(({ type: "TEXT", text: "x", textStyle: { fontVariant: "all-small-caps" } }));
  assert.equal((await figma.getNodeByIdAsync(smallCaps.id)).textCase, "SMALL_CAPS_FORCED");
  // "none" is the shared clearing word — it restores ORIGINAL casing, small-caps included.
  const cleared = await render(({ type: "TEXT", text: "x", textStyle: { textTransform: "none" } }));
  assert.equal((await figma.getNodeByIdAsync(cleared.id)).textCase, "ORIGINAL");
});

test("a run delta carries casing and paragraph metrics over its own slice", async () => {
  const out = await render(({ type: "TEXT", text: ["plain ", ["SHOUT", { textTransform: "uppercase", paragraphSpacing: 20 }]] }));
  const node = await figma.getNodeByIdAsync(out.id);
...
test("edit rides the same compile — a live node takes the new words too", async () => {
  const out = await render(({ type: "TEXT", text: "hi", key: "label" }));
...
test("a spread read textStyle compiles: derived leaves drop, unwritable ones fail loud by name", () => {
  ...
  const wn = compileTree(({ type: "TEXT", text: "x", textStyle: { fontVariantName: "Bold Italic", fontWeight: 700, fontStyle: "italic", fontSize: 18 } as never }), "spec");
  ...
  assert.throws(
    () => compileTree(({ type: "TEXT", text: "x", textStyle: { opentypeFlags: { LNUM: 1 } } as never }), "spec"),
  ...
  assert.throws(() => compileTree(({ type: "TEXT", text: "x", textStyle: { fontVariantname: "Bold" } as never }), "spec"), /unknown prop "fontVariantname"/);
});

test("a hyperlink takes the author string or the read form; a NODE link has no authored form", async () => {
  const out = await render(({ type: "TEXT", text: "docs", textStyle: { hyperlink: { type: "URL", url: "https://x.co" } } }));
  const node = await figma.getNodeByIdAsync(out.id);
  assert.deepEqual(node._rangeHyperlinks[0].value, { type: "URL", value: "https://x.co" });
  assert.throws(
    () => compileTree(({ type: "TEXT", text: "x", textStyle: { hyperlink: { type: "NODE", nodeID: "1:2" } } as never }), "spec"),
