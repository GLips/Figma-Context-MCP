...
async function renderRow() {
  const out = await render(
    ({ type: "FRAME", key: "row", width: 300, height: 100, layout: { mode: "row", gap: 10, padding: 5 }, children: [
...
test("append builds a node into the destination and sizes it THERE — fill fills the live parent", async () => {
  ...
  const out = await append("row", ({ type: "RECTANGLE", key: "filler", name: "filler", width: "fill", height: 20 }));
...
test("a moved node's fill is re-aimed at the new parent's axes, not left on the old one's", async () => {
  const out = await render(
    ({ type: "FRAME", key: "page-root", width: 400, height: 400, children: [
      ({ type: "FRAME", key: "row", width: 300, height: 100, layout: { mode: "row" }, children: [
        ({ type: "RECTANGLE", key: "grower", width: "fill", height: 20 }),
      ] }),
...
test("legality is re-asked against the DESTINATION: a fill can't land on the page", async () => {
  ...
  await assert.rejects(append(id(figma.currentPage.id), { id: (await resolveTarget("grower")).id }), /parent frame/);
...
test("the destination is read at the seal: an anchor moved during the image fetch is followed to its new parent", async () => {
  const out = await render(
    ({ type: "FRAME", key: "board", width: 400, height: 400, children: [
      ({ type: "FRAME", key: "left", width: 200, height: 200, children: [({ type: "RECTANGLE", key: "anchor", width: 40, height: 40 })] }),
  ...
  try {
    const placed = await insertAfter("anchor", ({ type: "RECTANGLE", name: "beside", width: 20, height: 20, fill: image("https://cdn.example.com/a.jpg") }));
  } finally {
    delete g.__flcmHost;
  }
...
test("a source deleted while the destination resolved refuses at the seal — Figma would have cloned the corpse", async () => {
  const out = await render(
    ({ type: "FRAME", key: "board", width: 400, height: 400, children: [({ type: "RECTANGLE", key: "src", width: 40, height: 40 }), ({ type: "FRAME", key: "dest", width: 200, height: 200 })] }),
  );
...
test("a destination dragged to a page this call never loaded refuses before the seal", async () => {
  const out = await render(({ type: "FRAME", key: "board", width: 400, height: 400, children: [({ type: "RECTANGLE", key: "anchor", width: 40, height: 40 })] }));
  const anchor = await figma.getNodeByIdAsync(specNode(out, "anchor").id);
  const before = [...figma.undoLog];
  const g = globalThis as { __flcmHost?: unknown };
  g.__flcmHost = {
  ...
  try {
    await assert.rejects(
      insertAfter("anchor", ({ type: "RECTANGLE", width: 20, height: 20, fill: image("https://cdn.example.com/a.jpg") })),
...
test("prepare rejects a non-container destination, a cycle, and a hand-built node — with zero writes", async () => {
  const out = await render(
    ({ type: "FRAME", key: "row", width: 300, height: 100, children: [
      ({ type: "RECTANGLE", key: "a", width: 40, height: 40 }),
  ...
  await assert.rejects(append("row", { id: (await resolveTarget("row")).id }), /can't be placed inside itself/);
  await assert.rejects(append("inner", { id: (await resolveTarget("row")).id }), /its own descendant/);
  await assert.rejects(append("row", { type: "FRAME", children: [{ type: "RECTANGLE", fills: [] }] } as never), /children\[0\].*fills/);
...
test("remove deletes the subtree and reports the id plus the reflowed parent", async () => {
  const out = await render(
    ({ type: "FRAME", key: "row", layout: { mode: "row", gap: 10 }, children: [
      ({ type: "RECTANGLE", key: "a", width: 40, height: 40 }),
      ({ type: "RECTANGLE", key: "b", width: 40, height: 40 }),
    ] }),
  );
  ...
  assert.deepEqual(row.children.map((c) => c.id), [specNode(out, "b").id]);
...
test("clone duplicates an INSTANCE-bearing subtree and strips every flcm/key from the copy", async () => {
  ...
  const src = await render(({ type: "FRAME", key: "badge", width: 60, height: 60, children: [({ type: "RECTANGLE", width: 20, height: 20 })] }));
  const component = figma.createComponentFromNode(await figma.getNodeByIdAsync(specNode(src, "badge").id));
  const out = await render(
    ({ type: "FRAME", key: "card", width: 200, height: 200, children: [({ type: "TEXT", text: "Hi", key: "title" })] }),
  );
...
test("an instance's CHILD LIST is closed, but the instance itself is an ordinary node", async () => {
  const out = await render(
    ({ type: "FRAME", key: "src", width: 60, height: 60, children: [({ type: "FRAME", key: "slot", width: 20, height: 20 })] }),
  );
  const src = await figma.getNodeByIdAsync(specNode(out, "src").id);
  ...
  await assert.rejects(append(id(instance.id), ({ type: "RECTANGLE" })), /inside component instance "Card instance"/);
  await assert.rejects(append(id(instance.children[0].id), ({ type: "RECTANGLE" })), /inside component instance "Card instance"/);
  await assert.rejects(append("dest", { id: instance.children[0].id }), /cannot move a component instance/);
...
test("clone with no parent lands the copy on the original's own coordinates", async () => {
  const out = await render(({ type: "FRAME", key: "board", width: 400, height: 400, children: [
    ({ type: "RECTANGLE", key: "chip", width: 40, height: 40, left: 30, top: 50 }),
  ] }));
...
test("measure returns parent-relative numeric geometry without mutation", async () => {
  const out = await render(({ type: "FRAME", width: 400, height: 200, left: 100, top: 70, children: [({ type: "RECTANGLE", key: "measured", width: 42, height: 23, left: 18, top: 9 })] }));
...
test("clone root overrides fix geometry before destination fill settlement and apply styles", async () => {
  await render(({ type: "FRAME", key: "source", width: 200, height: 100, layout: { mode: "row" }, children: [({ type: "RECTANGLE", key: "copy", width: "fill", height: 30, fill: "#ff0000" })] }));
  await render(({ type: "FRAME", key: "destination", width: 500, height: 100, layout: { mode: "row" } }));
...
test("replace preserves index and parent-relative constraints with explicit sizing precedence", async () => {
  const parent = await render(({ type: "FRAME", key: "replace-parent", width: 400, height: 300, children: [
    ({ type: "RECTANGLE", key: "before", width: 10, height: 10 }),
    ({ type: "RECTANGLE", key: "old", width: 90, height: 60, left: 23, top: 37, pin: { x: "right", y: "bottom" } }),
    ({ type: "RECTANGLE", key: "after", width: 10, height: 10 }),
  ] }));
  const old = (await get("old")).node.id;
  const out = await replace("old", ({ type: "RECTANGLE", name: "replacement", width: 120, fill: "#00ff00" }));
...
test("replace keeps fill and rejects incompatible inherited hug before writes", async () => {
  await render(({ type: "FRAME", key: "replace-row", width: 300, height: 100, layout: { mode: "row" }, children: [({ type: "RECTANGLE", key: "old-fill", width: "fill", height: 40 })] }));
  ...
  await render(({ type: "FRAME", key: "hug-old", layout: { mode: "row" }, children: [({ type: "RECTANGLE", width: 50, height: 20 })] }));
...
test("replace removal failure stays inside one rollback boundary", async () => {
  ...
  await assert.rejects(replace("replacement-failure", ({ type: "RECTANGLE", width: 30 })), /rolled back.*entry seal/);
