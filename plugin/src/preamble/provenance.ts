...
const readNodes = new WeakSet<object>();

// Brand what `get` hands back. A `get` result and a handle both carry a live string `id` — deliberate,
...
export function markReadNode(node: object): void {
  readNodes.add(node);
}

export function isReadNode(value: object): boolean {
  return readNodes.has(value);
}
