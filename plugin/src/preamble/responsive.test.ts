...
test("percent w/h resolves to a fraction of a free-form parent's fixed size", async () => {
  const out = await render({
    ...
    width: 300,
    height: 200,
    children: [{ type: "RECTANGLE", key: "bar", width: "35%", height: "50%" }],
  });
    ...
    children: [
      {
        ...
        width: "50%",
        height: 100,
        children: [{ type: "RECTANGLE", key: "leaf", width: "50%", height: 20 }],
      },
    ],
  });
...
test("percent absolute x/y resolves against the parent box", async () => {
  const out = await render({
    ...
    width: 200,
    height: 100,
    children: [{ type: "RECTANGLE", key: "badge", width: 40, height: 40, left: "50%", top: "50%" }],
  });
...
test("percent absolute position works inside an auto-layout parent (out of flow)", async () => {
  const out = await render({
    ...
    width: 200,
    height: 100,
    children: [{ type: "RECTANGLE", key: "pin", width: 20, height: 20, left: "25%" }],
  });
...
test("percent SIZE on an IN-FLOW child of a FIXED auto-layout parent resolves (static-only)", async () => {
  ...
  const out = await render({
    ...
    width: 300,
    height: 40,
    children: [{ type: "RECTANGLE", key: "half", width: "50%", height: 20 }],
  });
...
test("percent SIZE on an IN-FLOW child of a HUGGING auto-layout parent fails loud (true cycle)", async () => {
  ...
  await assert.rejects(
    render({
      type: "FRAME",
      layout: { mode: "column" },
      children: [{ type: "RECTANGLE", width: "50%" }],
    }), // column, no width → hugs width
    /cycle/,
  );
});
    ...
    children: [
      {
        ...
        width: "fill",
        height: 8,
        children: [{ type: "RECTANGLE", key: "play", width: "40%", height: 8 }],
      },
    ],
  });
...
test("percent-width TEXT wraps at the resolved width (not silently ignored)", async () => {
  ...
  const out = await render({
    ...
    children: [
      {
        type: "TEXT",
        text: "a fairly long headline that would overflow if it never wrapped",
        key: "t",
        width: "50%",
      },
    ],
...
test("percent SIZE on an ABSOLUTE (out-of-flow) child resolves inside an auto-layout parent", async () => {
  ...
  const out = await render({
    ...
    width: 300,
    height: 100,
    children: [{ type: "RECTANGLE", key: "overlay", width: "50%", height: 40, left: 10, top: 10 }],
  });
...
test("anchor works with a numeric position too (no percent required)", async () => {
  const out = await render({
    ...
    height: 100,
    children: [
      { type: "RECTANGLE", key: "dot", width: 10, height: 10, left: 100, anchor: { x: "center" } },
    ],
  });
  assert.equal((await figma.getNodeByIdAsync(specNode(out, "dot").id)).x, 95); // 100 - half of 10
});

test("a bad anchor value fails loud", async () => {
  await assert.rejects(
    render({ type: "RECTANGLE", left: 0, anchor: { x: "top" } } as never),
    /anchor\.x must be one of/,
  );
  await assert.rejects(
    render({ type: "RECTANGLE", left: 0, anchor: { y: "left" } } as never),
    /anchor\.y must be one of/,
  );
});
...
test('free-form width:"fill" actually stretches to the parent box (no warn-and-ignore)', async () => {
  const out = await render({
    ...
    width: 300,
    height: 200,
    children: [{ type: "RECTANGLE", key: "bar", width: "fill", height: 12 }],
  });
  ...
  const out = await render({
    ...
    children: [
      { type: "RECTANGLE", key: "r", width: "fill", height: 40, pin: { x: "right", y: "bottom" } }, // over fill/MIN
      {
        ...
        left: 10,
        top: 10,
        pin: { x: "center", y: "scale" },
      },
    ],
  });
  assert.deepEqual((await figma.getNodeByIdAsync(specNode(out, "r").id)).constraints, {
    horizontal: "MAX",
    vertical: "MAX",
  });
  assert.deepEqual((await figma.getNodeByIdAsync(specNode(out, "c").id)).constraints, {
    horizontal: "CENTER",
    vertical: "SCALE",
  });
});
...
test("an IN-FLOW auto-layout child STORES an explicit pin (inert in flow, governs if it leaves)", async () => {
  ...
  const out = await render({
    ...
    width: 300,
    height: 80,
    children: [{ type: "RECTANGLE", key: "grow", width: "fill", height: 40, pin: { x: "right" } }],
  });
...
test("an ABSOLUTE child of an auto-layout parent honors pin/constraints (out of flow)", async () => {
  ...
  assert.deepEqual((await figma.getNodeByIdAsync(specNode(out, "badge").id)).constraints, {
    horizontal: "MAX",
    vertical: "MIN",
  });
});

test("a bad `pin` value fails loud", async () => {
  await assert.rejects(
    render({ type: "RECTANGLE", pin: { x: "middle" } } as never),
    /pin\.x must be one of/,
  );
  await assert.rejects(
    render({ type: "RECTANGLE", pin: { y: "left" } } as never),
    /pin\.y must be one of/,
  );
  await assert.rejects(
    render({ type: "RECTANGLE", pin: "right" } as never),
