...
test('percent w/h resolves to a fraction of a free-form parent\'s fixed size', async () => {
  const out = await render(
    ({ type: "FRAME", width: 300, height: 200, children: [
      ({ type: "RECTANGLE", key: "bar", width: "35%", height: "50%" }),
    ] }),
  );
  const bar = await figma.getNodeByIdAsync(specNode(out, "bar").id);
...
test("percent resolves against a percent-sized ancestor (chained)", async () => {
  const out = await render(
    ({ type: "FRAME", width: 400, children: [
      ({ type: "FRAME", key: "mid", width: "50%", height: 100, children: [
        ({ type: "RECTANGLE", key: "leaf", width: "50%", height: 20 }),
      ] }),
    ] }),
  );
  assert.equal((await figma.getNodeByIdAsync(specNode(out, "mid").id)).width, 200); // 50% of 400
  assert.equal((await figma.getNodeByIdAsync(specNode(out, "leaf").id)).width, 100); // 50% of the resolved 200
});

test("percent absolute x/y resolves against the parent box", async () => {
  const out = await render(
    ({ type: "FRAME", width: 200, height: 100, children: [
      ({ type: "RECTANGLE", key: "badge", width: 40, height: 40, left: "50%", top: "50%" }),
    ] }),
  );
...
test("percent absolute position works inside an auto-layout parent (out of flow)", async () => {
  const out = await render(
    ({ type: "FRAME", layout: { mode: "row" }, width: 200, height: 100, children: [
      ({ type: "RECTANGLE", key: "pin", width: 20, height: 20, left: "25%" }),
    ] }),
  );
  const pin = await figma.getNodeByIdAsync(specNode(out, "pin").id);
...
test("percent SIZE on an IN-FLOW child of a FIXED auto-layout parent resolves (static-only)", async () => {
  ...
  const out = await render(
    ({ type: "FRAME", layout: { mode: "row" }, width: 300, height: 40, children: [({ type: "RECTANGLE", key: "half", width: "50%", height: 20 })] }),
  );
  assert.equal((await figma.getNodeByIdAsync(specNode(out, "half").id)).width, 150); // 50% of the fixed 300
});
...
test("percent SIZE on an IN-FLOW child of a HUGGING auto-layout parent fails loud (true cycle)", async () => {
  ...
  await assert.rejects(
    render(({ type: "FRAME", layout: { mode: "column" }, children: [({ type: "RECTANGLE", width: "50%" })] })), // column, no width → hugs width
    /cycle/,
  );
});
...
test('percent SIZE resolves against a "fill" parent\'s realized size (the scrub-bar case)', async () => {
  ...
  const out = await render(
    ({ type: "FRAME", layout: { mode: "row" }, width: 400, height: 20, children: [
      ({ type: "FRAME", key: "track", width: "fill", height: 8, children: [({ type: "RECTANGLE", key: "play", width: "40%", height: 8 })] }),
    ] }),
  );
  assert.equal((await figma.getNodeByIdAsync(specNode(out, "track").id)).width, 400); // fills the row
  assert.equal((await figma.getNodeByIdAsync(specNode(out, "play").id)).width, 160); // 40% of the realized 400
});
...
test("percent-width TEXT wraps at the resolved width (not silently ignored)", async () => {
  ...
  const out = await render(
    ({ type: "FRAME", width: 300, height: 200, children: [
      ({ type: "TEXT", text: "a fairly long headline that would overflow if it never wrapped", key: "t", width: "50%" }),
    ] }),
  );
  assert.equal((await figma.getNodeByIdAsync(specNode(out, "t").id)).width, 150); // 50% of 300, wrapping
});
...
test("percent SIZE on an ABSOLUTE (out-of-flow) child resolves inside an auto-layout parent", async () => {
  ...
  const out = await render(
    ({ type: "FRAME", layout: { mode: "row" }, width: 300, height: 100, children: [
      ({ type: "RECTANGLE", key: "overlay", width: "50%", height: 40, left: 10, top: 10 }),
    ] }),
  );
  assert.equal((await figma.getNodeByIdAsync(specNode(out, "overlay").id)).width, 150); // 50% of 300
});

test("percent on the root node (no parent) fails loud", async () => {
  await assert.rejects(render(({ type: "RECTANGLE", width: "50%" })), /parent is the page/);
});
...
test("anchor centres a child on the resolved point (no half-width offset)", async () => {
  const out = await render(
    ({ type: "FRAME", width: 200, height: 100, children: [
      ({ type: "RECTANGLE", key: "knob", width: 40, height: 40, left: "50%", top: "50%", anchor: { x: "center", y: "center" } }),
    ] }),
  );
  const knob = await figma.getNodeByIdAsync(specNode(out, "knob").id);
...
test("anchor right/bottom pins a child's far edge to the point", async () => {
  const out = await render(
    ({ type: "FRAME", width: 300, height: 200, children: [
      ({ type: "RECTANGLE", key: "badge", width: 24, height: 24, left: "100%", top: 0, anchor: { x: "right", y: "top" } }),
    ] }),
  );
  const badge = await figma.getNodeByIdAsync(specNode(out, "badge").id);
...
test("anchor works with a numeric position too (no percent required)", async () => {
  const out = await render(
    ({ type: "FRAME", width: 200, height: 100, children: [({ type: "RECTANGLE", key: "dot", width: 10, height: 10, left: 100, anchor: { x: "center" } })] }),
  );
  assert.equal((await figma.getNodeByIdAsync(specNode(out, "dot").id)).x, 95); // 100 - half of 10
});

test("a bad anchor value fails loud", () => {
  assert.throws(() => compileTree(({ type: "RECTANGLE", ...({ left: 0, anchor: { x: "top" } } as never) }), "spec"), /anchor\.x must be one of/);
  assert.throws(() => compileTree(({ type: "RECTANGLE", ...({ left: 0, anchor: { y: "left" } } as never) }), "spec"), /anchor\.y must be one of/);
});
...
test("a free-form child's constraints derive from its size/position intent", async () => {
  const out = await render(
    ({ type: "FRAME", width: 300, height: 200, children: [
      ({ type: "RECTANGLE", key: "plain", width: 40, height: 40 }),                                    // numeric → MIN (Figma default)
      ({ type: "RECTANGLE", key: "fillW", width: "fill", height: 40 }),                                // fill → STRETCH
      ({ type: "RECTANGLE", key: "pctW", width: "50%", height: 40 }),                                  // percent size → SCALE
      ({ type: "RECTANGLE", key: "pctPos", width: 40, height: 40, left: "50%", top: "50%" }), // percent position → CENTER
      ({ type: "RECTANGLE", key: "numPos", width: 40, height: 40, left: 20, top: 20 }),       // numeric position → MIN
    ] }),
  );
  const c = async (k: string) => (await figma.getNodeByIdAsync(specNode(out, k).id)).constraints;
...
test('free-form width:"fill" actually stretches to the parent box (no warn-and-ignore)', async () => {
  const out = await render(({ type: "FRAME", width: 300, height: 200, children: [({ type: "RECTANGLE", key: "bar", width: "fill", height: 12 })] }));
  assert.equal((await figma.getNodeByIdAsync(specNode(out, "bar").id)).width, 300);
});

test("`pin` overrides the auto-derived constraint per axis", async () => {
  const out = await render(
    ({ type: "FRAME", width: 300, height: 200, children: [
      ({ type: "RECTANGLE", key: "r", width: "fill", height: 40, pin: { x: "right", y: "bottom" } }),           // over fill/MIN
      ({ type: "RECTANGLE", ...({ key: "c", width: 40, height: 40, left: 10, top: 10, pin: { x: "center", y: "scale" } } as never) }),
    ] }),
  );
  assert.deepEqual((await figma.getNodeByIdAsync(specNode(out, "r").id)).constraints, { horizontal: "MAX", vertical: "MAX" });
  assert.deepEqual((await figma.getNodeByIdAsync(specNode(out, "c").id)).constraints, { horizontal: "CENTER", vertical: "SCALE" });
});
...
test("an IN-FLOW auto-layout child STORES an explicit pin (inert in flow, governs if it leaves)", async () => {
  ...
  const out = await render(
    ({ type: "FRAME", layout: { mode: "row" }, width: 300, height: 80, children: [
      ({ type: "RECTANGLE", ...({ key: "grow", width: "fill", height: 40, pin: { x: "right" } } as never) }),
    ] }),
  );
  const node = await figma.getNodeByIdAsync(specNode(out, "grow").id);
...
test("an ABSOLUTE child of an auto-layout parent honors pin/constraints (out of flow)", async () => {
  ...
  const out = await render(
    ({ type: "FRAME", layout: { mode: "row" }, width: 320, height: 200, children: [
      ({ type: "RECTANGLE", ...({ key: "badge", width: 28, height: 28, left: 284, top: 12, pin: { x: "right", y: "top" } } as never) }),
    ] }),
  );
  assert.deepEqual((await figma.getNodeByIdAsync(specNode(out, "badge").id)).constraints, { horizontal: "MAX", vertical: "MIN" });
});

test("a bad `pin` value fails loud", () => {
  assert.throws(() => compileTree(({ type: "RECTANGLE", ...({ pin: { x: "middle" } } as never) }), "spec"), /pin\.x must be one of/);
  assert.throws(() => compileTree(({ type: "RECTANGLE", ...({ pin: { y: "left" } } as never) }), "spec"), /pin\.y must be one of/);
  assert.throws(() => compileTree(({ type: "RECTANGLE", ...({ pin: "right" } as never) }), "spec"), /pin must be an object/);
});
