...
test("wrap and unequal CSS gaps create, edit, inherit and round-trip", async () => {
  ...
  const built = await render({
    type: "FRAME",
    width: 220,
    layout: { mode: "row", wrap: true, gap: "12px 24px" },
    children: [
      { type: "RECTANGLE", width: 80 },
      { type: "RECTANGLE", width: 80 },
      { type: "RECTANGLE", width: 80 },
    ],
  });
  ...
  assert.equal(read.node.layout.wrap, true);
  await edit(id(native.id), { layout: { gap: 6 } });
  assert.equal(native.itemSpacing, 6);
  assert.equal(native.counterAxisSpacing, 6);
  ...
  await assert.rejects(
    render({ type: "FRAME", layout: { mode: "row", wrap: true, gap: "-1px 8px" } }),
    /non-negative/,
  );
  const comp = await component({
    type: "FRAME",
    width: 200,
    layout: { mode: "row", wrap: true, gap: "5px 9px" },
  });
  ...
  assert.equal(inst.counterAxisSpacing, 7);
  assert.equal(inst.itemSpacing, 11);
});
...
test("new text fills independently bounded columns; explicit sizing and edits are preserved", async () => {
  ...
  const tree = await render({
    ...
    children: [
      {
        type: "TEXT",
        text: "A long paragraph that should wrap rather than grow the column",
        key: "implicit",
      },
  ...
  assert.notEqual((await figma.getNodeByIdAsync(specNode(tree, "hug").id)).layoutAlign, "STRETCH");
  const hugColumn = await render({
    type: "FRAME",
    layout: { mode: "column" },
    children: [
      { type: "TEXT", text: "Content", key: "content" },
      {
        type: "FRAME",
        width: "fill",
        layout: { mode: "column" },
        children: [{ type: "TEXT", text: "Nested content", key: "nested-content" }],
      },
    ],
  });
  for (const key of ["content", "nested-content"])
    assert.notEqual(
      (await figma.getNodeByIdAsync(specNode(hugColumn, key).id)).layoutAlign,
      "STRETCH",
    );
  const row = await render({
    ...
    width: 240,
    layout: { mode: "row" },
    children: [{ type: "TEXT", text: "Row default", key: "row-text" }],
  });
  assert.equal(
    (await figma.getNodeByIdAsync(specNode(row, "row-text").id)).textAutoResize,
    "WIDTH_AND_HEIGHT",
  );
});
...
test("nested exposure is applied after promotion and placement, and rejected outside a definition", async () => {
  ...
  await assert.rejects(
    render({ type: "INSTANCE", componentId: inner.id, exposed: true }),
    /exposed needs/,
  );
  assert.equal(figma.currentPage.children.length, count);
  const outer = await component({
    type: "FRAME",
    children: [{ type: "INSTANCE", componentId: inner.id, key: "nested", exposed: true }],
  });
  ...
  const appended = await append(id(outer.id), {
    type: "INSTANCE",
