...
import { WritePaint, WriteGradient, GradientStop, GradientType, Rgba, Transform } from "./ir.js";

// A color (alpha included) -> SOLID paint, alpha mapped to the paint's opacity. A fully-transparent color
// becomes opacity 0, never the opaque-black bug a naive parse produces.
export function solid(color: Rgba): WritePaint {
  return { kind: "solid", color: { r: color.r, g: color.g, b: color.b }, opacity: color.a };
}

// CSS linear angle (deg) + stops -> linear gradient paint. The transform is derived here so the sugar and
// the string parser can never disagree on the matrix.
export function linearGradient(cssDeg: number, stops: GradientStop[]): WriteGradient {
...
export function radialGradient(type: GradientType, cx: number, cy: number, stops: GradientStop[]): WriteGradient {
...
// WritePaint -> the plugin's Paint. The bridge routes every fill/stroke through here; it is the single
// typed->plugin boundary for paint, so the one unavoidable plugin-typings cast lives here, not at call sites.
export function toFigmaPaint(paint: WritePaint): Paint {
  if (paint.kind === "solid") {
    return { type: "SOLID", color: paint.color, opacity: paint.opacity };
  }
  if (paint.kind === "image") {
    // An image paint needs raster bytes + figma.createImage — a figma.* call, so it resolves in the bridge.
    // paintOf is the SOLE WritePaint→Paint funnel there and routes every image paint to its own resolver, so a
    // pure image paint never reaches this figma-free path; this narrows the union and fails loud if that ever
