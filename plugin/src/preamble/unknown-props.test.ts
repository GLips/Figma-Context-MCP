...
import { KNOWN_KEYS, DIRECTIONAL_KEYS, BINDING_FIELD_KEYS, NODE_KEYS_BY_TYPE, gradient, image, effects } from "./flcm.js";
...
test("fromRead's compiler key sets match the complete prop schemas", () => {
  ...
  for (const type of Object.keys(schemas) as (keyof typeof schemas)[]) {
    assert.deepEqual([...NODE_KEYS_BY_TYPE[type]].sort(), Object.keys(schemas[type].shape).sort(), type);
  }
});
...
test("verbs reject an unknown top-level prop, naming it and the verb", () => {
  assert.throws(() => compileTree(({ type: "FRAME", ...({ background: "#fff" } as never) }), "spec"), /unknown prop "background" on FRAME/);
  assert.throws(() => compileTree(({ type: "TEXT", text: "hi", ...({ textTransform: "upper" } as never) }), "spec"), /unknown prop "textTransform" on TEXT/);
  assert.throws(() => compileTree(({ type: "RECTANGLE", ...({ radius: 4 } as never) }), "spec"), /unknown prop "radius" on RECTANGLE/); // it's borderRadius
  assert.throws(() => compileTree(({ type: "LINE", ...({ height: 10 } as never) }), "spec"), /unknown prop "height" on LINE/); // a line sizes on width only
  assert.throws(() => compileTree(({ type: "VECTOR", ...({ d: "M0 0 L1 1", borderRadius: 2 } as never) }), "spec"), /unknown prop "borderRadius" on VECTOR/);
...
test("a non-object where a props/query object belongs names THAT mistake, not the string's indices", async () => {
  ...
  assert.throws(() => compileTree("red", "spec"), /plain node spec/);
  assert.throws(() => compileTree([1, 2], "spec"), /plain node spec/);
  assert.throws(() => compileTree(false, "spec"), /plain node spec/);

});

test("nested authoring objects reject unknown keys with a path-threaded error", () => {
  assert.throws(() => compileTree(({ type: "FRAME", layout: { mode: "row", flexWrap: "nowrap" } as never }), "spec"), /unknown prop "flexWrap" on FRAME\.layout/);
  assert.throws(() => compileTree(({ type: "TEXT", text: "hi", textStyle: { fontVarient: "small-caps" } as never }), "spec"), /unknown prop "fontVarient" on TEXT\.textStyle/);
  assert.throws(() => compileTree(({ type: "FRAME", left: 1, anchor: { z: "left" } as never }), "spec"), /unknown prop "z" on anchor/);
  assert.throws(() => compileTree(({ type: "FRAME", pin: { z: "left" } as never }), "spec"), /unknown prop "z" on pin/);
});
...
test("a run delta rejects an unknown key (the grounded silent-drop site), naming the run index", () => {
  ...
  assert.throws(
    () => compileTree(({ type: "TEXT", text: ["ok", ["styled", { textTransfrom: "uppercase" }] as never] }), "spec"),
...
test("the CSS effects bag (a node's `effects:` prop) rejects an unknown key too — surface-wide policy", () => {
  ...
  assert.throws(() => compileTree(({ type: "FRAME", effects: { boxShadow: shadow, foo: 2 } as never }), "spec"), /unknown prop "foo" on effects/);
  assert.throws(() => compileTree(({ type: "RECTANGLE", effects: { filter: "blur(4px)", bar: 1 } as never }), "spec"), /unknown prop "bar" on effects/);
  // Both vocabularies in one bag still parse — the shape `get` hands back.
  assert.doesNotThrow(() => compileTree(({ type: "FRAME", effects: { boxShadow: shadow, blur: 4 } }), "spec"));
});

test("plural offenders are all named", () => {
  assert.throws(() => compileTree(({ type: "FRAME", ...({ foo: 1, bar: 2 } as never) }), "spec"), /unknown props "foo", "bar" on FRAME/);
});
...
test("known props on every verb still construct (the reject is closed, not over-broad)", () => {
  ...
  assert.doesNotThrow(() =>
    compileTree(({ type: "FRAME", name: "n", width: 100, height: "hug", fill: "#fff", layout: { mode: "row", gap: 8, padding: 4 }, left: 1, top: 2, anchor: { x: "center" }, pin: { x: "left" } }), "spec"),
  );
  assert.doesNotThrow(() => compileTree(({ type: "TEXT", text: "hi", fill: "#000", textStyle: { fontSize: 14, fontWeight: "bold", textAlign: "center" } }), "spec"));
  assert.doesNotThrow(() => compileTree(({ type: "TEXT", text: [["b", { fontWeight: "bold", color: "#f00", hyperlink: "https://x" }]] }), "spec"));
  assert.doesNotThrow(() => compileTree(({ type: "RECTANGLE", fill: "#fff", borderRadius: 4, strokeWidth: 1 }), "spec"));
  assert.doesNotThrow(() => compileTree(({ type: "LINE", stroke: "#000", width: 40, strokeWidth: 2 }), "spec"));
  assert.doesNotThrow(() => compileTree(({ type: "VECTOR", d: "M0 0 L1 1", fill: "#000" }), "spec"));
  assert.doesNotThrow(() => compileTree(({ type: "VECTOR", svg: '<svg viewBox="0 0 1 1"></svg>', width: 24 }), "spec"));
