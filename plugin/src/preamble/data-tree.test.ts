...
test("plain templates stamp repeatedly; every returned node receives an id without aliasing input", async () => {
  ...
  const template = { type: "FRAME", width: 100, height: 100, children: [{ type: "RECTANGLE", width: 20, height: 20, annotations: [{ text: "Keep this note" }] }] };
  const before = JSON.stringify(template);
  const a = await flcm.render(template), b = await flcm.render(template);
...
test("get node data authors as-is: move recursively, edit props, keep omitted children", async () => {
  const figma = createFigmaMock();
  const root = await flcm.render({ type: "FRAME", width: 200, height: 100, children: [{ type: "TEXT", text: "Existing" }] });
...
test("a created parent can move an existing child and create its sibling", async () => {
  ...
  const result = await flcm.render({ type: "FRAME", width: 100, height: 100, children: [{ id: old.id, fill: "#00FF00" }, { type: "TEXT", text: "New" }] });
...
test("placements return input data with ids; before, after, prepend and replace order", async () => {
  ...
  assert.deepEqual((await figma.getNodeByIdAsync(root.id) as any).children.map((n: any) => n.id), [d.id, a.id, b.id]);
...
test("deep malformed trees and duplicate live ids refuse before writes with their paths", async () => {
  ...
  await assert.rejects(flcm.render({ type: "FRAME", children: [{ type: "FRAME", children: [{ type: "RECTANGLE", background: "red" }] }] }), /children\[0\].children\[0\].*background/);
  await assert.rejects(flcm.render({ type: "FRAME", children: [{ id: old.id }, { id: old.id }] }), /children\[1\].id.*duplicate/);
  await assert.rejects(flcm.render({ type: "FRAME", children: [{ id: "bad:id" }] }), /children\[0\]/);
...
test("read-only vocabulary refuses by field and annotations survive a data copy", async () => {
  createFigmaMock();
  for (const words of [{ strokeDashes: [1, 2] }, { aspectRatio: 2 }, { fill: "fill_a1b2c3d4" }, { layout: { mode: "grid" } }]) {
    await assert.rejects(flcm.render({ type: "FRAME", ...words }), /strokeDashes|aspectRatio|REFERENCE|grid/);
  }
  const original = await flcm.render({ type: "RECTANGLE", width: 25, height: 30, annotations: [{ text: "Intent" }] });
...
test("IR words are rejected and no node constructors, fromRead or move are exported", async () => {
  createFigmaMock();
  for (const name of ["frame", "text", "rect", "ellipse", "line", "svg", "path", "instance", "fromRead", "move"]) assert.equal(name in flcm, false);
  await assert.rejects(flcm.render({ type: "RECTANGLE", fills: [] }), /fills/);
  const cycle: NodeSpec = { type: "FRAME" }; cycle.children = [cycle];
...
test("replace can wrap the target it retains as an id-bearing child", async () => {
  ...
  const wrapped = await flcm.replace(target, { type: "FRAME", width: 100, height: 100, children: [{ id: target.id }] });
...
test("resource and apply-time errors identify the node's indexed path", async () => {
  createFigmaMock();
  await assert.rejects(flcm.render({ type: "FRAME", children: [{ type: "INSTANCE", componentId: "1:999999" }] }), /spec.children\[0\]/);
  await assert.rejects(flcm.render({ type: "FRAME", children: [{ type: "VECTOR", d: "bad path" }] }), /spec.children\[0\]/);
});
...
test("duplicate authored keys across live and new nodes fail before writing", async () => {
  ...
  await assert.rejects(flcm.render({ type: "FRAME", children: [{ id: old.id, key: "same" }, { type: "TEXT", text: "Hi", key: "same" }] }), /children\[1\].key.*duplicate/);
...
test("returned path data can move and edit its live vector without changing identity", async () => {
  const figma = createFigmaMock();
  const vector = await flcm.render({ type: "VECTOR", d: "M0 0 L10 10", width: 24, height: 24, stroke: "#111" });
...
test("a promoted frame's returned data can be placed again", async () => {
  const figma = createFigmaMock();
  const spec = await flcm.component({ type: "FRAME", width: 100, height: 100, children: [{ type: "TEXT", text: "Label" }] });
...
test("moved nodes bind to properties in their destination component", async () => {
  const figma = createFigmaMock();
  const component = await flcm.component({ type: "FRAME", width: 100, height: 100 }, { propertyDefinitions: { Label: { type: "text", defaultValue: "Default" } } });
...
test("component promotion can declare bindings on existing children and derive their values", async () => {
  ...
  const component = await flcm.component({ type: "FRAME", width: 100, height: 100, children: [{ id: text.id, componentPropertyReferences: { text: "Label" } }] }, { propertyDefinitions: { Label: { type: "text" } } });
