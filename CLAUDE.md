...
`plugin/src/preamble/schema.ts` is the single source of the `flcm` authoring surface — types, validation, and generated docs all derive from it. `src/services/plugin-bridge/` is the WS relay to the plugin (port block, approval gate, version-skew policy, trusted image fetch), and `src/mcp/tools/code-mode-tools.ts` registers `figma_execute_code`, `get_flcm_reference`, `get_screenshot`.
